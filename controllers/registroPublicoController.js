...
export const registrarEmpleadoPublico = async (req, res) => {
  try {
    ...
    if (!tokenData || tokenData.length === 0) {
      return res.status(404).json({ message: "Token no encontrado o inválido." });
    }
    ...
    if (!url_hoja_de_vida || !url_cedula || !url_certificado_bancario || !url_habeas_data) {
      return res.status(400).json({
        message: "Todos los documentos son obligatorios (CV, Cédula, Cert. Bancario y Habeas Data).",
      });
    }
    ...
    const payload = {
      tipo: "empleado",
      estado: "pendiente",
      user_id: tokenInfo.generado_por, // Usuario que generó el token
      token: token,
      datos: {
        nombre,
        apellidos,
    ...
    console.error("Error en registrarEmpleadoPublico:", error.response?.data || error.message);
    
    // Manejo específico de error de cédula duplicada
...
export const registrarClientePublico = async (req, res) => {
  try {
    ...
    // Validaciones
    if (!url_rut || !cupo || !plazo || !url_camara_comercio || !url_formato_sangrilaft || !url_cedula) {
      return res.status(400).json({ 
    ...
    console.error("Error en registrarClientePublico:", error.response?.data || error.message);
    res.status(500).json({
      message: "Error al procesar el registro.",
      error: error.message,
    });
...
export const registrarProveedorPublico = async (req, res) => {
  try {
    ...
    // Validaciones
    if (!url_rut || !url_certificacion_bancaria || !url_formato_vinculacion || !url_composicion_accionaria) {
      return res.status(400).json({
        message: "Faltan documentos obligatorios (RUT, Cert. Bancaria, Vinculación y Comp. Accionaria).",
      });
    }
    ...
    console.error("Error en registrarProveedorPublico:", error.response?.data || error.message);
    res.status(500).json({
      message: "Error al procesar el registro.",
      error: error.message,
    });
