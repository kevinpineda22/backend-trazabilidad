...
export const createProveedorContabilidad = async (req, res) => {
    try {
        ...
        if (!rutFile || !camaraComercioFile || !certificacionBancariaFile) {
             return res.status(400).json({ message: "Faltan documentos obligatorios (RUT, Cámara de Comercio, Certificación Bancaria)." });
        }
        ...
        const [
            ...
            // Obligatorios
            uploadFileToStorage(rutFile, 'documentos_contabilidad', `${basePath}/rut.${getFileExtension(rutFile.originalname)}`),
            uploadFileToStorage(camaraComercioFile, 'documentos_contabilidad', `${basePath}/camara_comercio.${getFileExtension(camaraComercioFile.originalname)}`),
            uploadFileToStorage(certificacionBancariaFile, 'documentos_contabilidad', `${basePath}/certificacion_bancaria.${getFileExtension(certificacionBancariaFile.originalname)}`),
            
            // Opcionales
            uploadFileToStorage(formatoVinculacionFile, 'documentos_contabilidad', `${basePath}/formato_vinculacion.${getFileExtension(formatoVinculacionFile?.originalname)}`),
            uploadFileToStorage(composicionAccionariaFile, 'documentos_contabilidad', `${basePath}/composicion_accionaria.${getFileExtension(composicionAccionariaFile?.originalname)}`),
        ]);
