...
import { FaArrowLeft, FaUserCircle, FaFilePdf, FaFileImage, FaFileAlt, FaInfoCircle, FaDownload } from 'react-icons/fa';
...
const ExpedienteEmpleadoView = ({ empleadoId, onBack, onPreview }) => {
    ...
    useEffect(() => {
        ...
        const fetchExpediente = async () => {
            try {
                ...
                const { data } = await api.get(`/trazabilidad/empleados/admin/expediente/${empleadoId}`);
                setEmpleado(data.empleado); // ¡Solo guardamos el empleado!
                setError(null);
            } catch (err) {
                console.error("Error fetching expediente:", err);
                const errorMsg = err.response?.data?.message || "No se pudo cargar el expediente.";
                setError(errorMsg);
    ...
    const getFileIcon = (fileNameOrUrl) => {
        ...
        if (fileNameOrUrl.toLowerCase().endsWith('.pdf')) return <FaFilePdf className="file-icon pdf" />;
        if (/\.(jpg|jpeg|png|gif|webp)$/i.test(fileNameOrUrl)) return <FaFileImage className="file-icon image" />;
        ...
        <>
            <div className="expediente-container-inner">
                    ...
                    <>
                        ...
                        <div className="expediente-header">
                            ...
                            <div className="header-info">
                                <h1 className="header-title">{empleado.nombre} {empleado.apellidos}</h1>
                        ...
                        <div className="expediente-info-card">
                            <h2 className="info-card-title">Información de Contacto y Registro</h2>
                            <div className="info-card-grid">
                                <InfoItem label="Correo Electrónico" value={empleado.correo_electronico} />
                                <InfoItem label="Contacto (Celular)" value={empleado.contacto} />
                                <InfoItem label="Dirección" value={empleado.direccion} />
                                <InfoItem label="Código Ciudad" value={empleado.codigo_ciudad} />
                                <InfoItem label="Código Departamento" value={empleado.codigo_departamento} />
                                <InfoItem label="Código País" value={empleado.codigo_pais} />
                                <InfoItem label="Creado por" value={empleado.profiles?.nombre} />
                                <InfoItem 
                                    label="Fecha Creación" 
                                    value={format(parseISO(empleado.created_at), 'dd/MM/yyyy hh:mm a')} 
                                />
                            </div>
                        </div>
                    </>
                ...
                <div className="expediente-file-list">
                    ...
                    {empleado && documentosDefinidos.filter(doc => doc.url).length > 0 ? (
                        <ul>
                            {documentosDefinidos
                                .filter(doc => doc.url) // Solo muestra documentos que SÍ tienen URL
                                .map((doc) => (
                                <li key={doc.label} className="file-item"> {/* Key ahora es el label */}
                        ...
                        !loading && !error && (
                            <div className="admin-cont-empty-message" style={{marginTop: '1rem'}}>
                                <FaInfoCircle />
