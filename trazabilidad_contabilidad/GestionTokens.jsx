...
const GestionTokens = ({ userRole }) => {
  ...
  const generarUrlRegistro = (tipo, token) => {
    const rutas = {
      empleado: "/autogestion", // Ruta unificada
  ...
  return (
    <div className="tokens-container">
      ...
      {mensaje && (
        <div
          className={`tokens-mensaje ${tipoMensaje === "success" ? "tokens-mensaje-exito" : "tokens-mensaje-error"}`}
        >
          {tipoMensaje === "success" ? (
            <FaCheckCircle />
          ) : (
      ...
      <div className="tokens-filtros-container">
        <div className="tokens-filtros-estado">
          <button
            className={`tokens-filtro-btn ${filtroEstado === "activo" ? "tokens-activo" : ""}`}
            onClick={() => setFiltroEstado("activo")}
          >
            <FaCheckCircle /> Activos ({contadorEstados.activo})
          </button>
          <button
            className={`tokens-filtro-btn ${filtroEstado === "usado" ? "tokens-activo" : ""}`}
            onClick={() => setFiltroEstado("usado")}
          >
            <FaCheck /> Usados ({contadorEstados.usado})
          </button>
          <button
            className={`tokens-filtro-btn ${filtroEstado === "expirado" ? "tokens-activo" : ""}`}
            onClick={() => setFiltroEstado("expirado")}
          >
            <FaClock /> Expirados ({contadorEstados.expirado})
          </button>
          <button
            className={`tokens-filtro-btn ${filtroEstado === "todos" ? "tokens-activo" : ""}`}
      ...
      <div className="tokens-tabla-wrapper">
        ...
        {loading && tokens.length === 0 ? (
          ...
          <div className="tokens-accordion-container">
            {puedeGestionar("empleado") && (
              <div
                className={`tokens-accordion-item ${seccionExpandida === "empleado" ? "expandido" : ""}`}
              >
                <button
                  className="tokens-accordion-header"
                  onClick={() => toggleSeccion("empleado")}
                >
                  <div className="header-left">
                    <span className="icon-box icon-empleado">
                      <FaUserTie />
                    </span>
                ...
                <div
                  className={`tokens-accordion-body ${seccionExpandida === "empleado" ? "abierto" : ""}`}
                >
                  {tokensUsadosAgrupados.empleado?.length > 0 ? (
                    <TablaTokens listaTokens={tokensUsadosAgrupados.empleado} />
                  ) : (
                    <div className="p-3 text-muted">
                      No hay registros usados.
                    </div>
                  )}
                </div>
              </div>
            ...
            {puedeGestionar("cliente") && (
              <div
                className={`tokens-accordion-item ${seccionExpandida === "cliente" ? "expandido" : ""}`}
              >
                <button
                  className="tokens-accordion-header"
                ...
                >
                  <div className="header-left">
                    <span className="icon-box icon-cliente">
                      <FaUser />
                    </span>
                ...
                <div
                  className={`tokens-accordion-body ${seccionExpandida === "cliente" ? "abierto" : ""}`}
                >
                  {tokensUsadosAgrupados.cliente?.length > 0 ? (
            ...
            {puedeGestionar("proveedor") && (
              <div
                className={`tokens-accordion-item ${seccionExpandida === "proveedor" ? "expandido" : ""}`}
              >
                <button
                  className="tokens-accordion-header"
                ...
                >
                  <div className="header-left">
                    <span className="icon-box icon-proveedor">
                      <FaTruck />
                    </span>
                ...
                <div
                  className={`tokens-accordion-body ${seccionExpandida === "proveedor" ? "abierto" : ""}`}
                >
                  {tokensUsadosAgrupados.proveedor?.length > 0 ? (
                    <TablaTokens
                      listaTokens={tokensUsadosAgrupados.proveedor}
                    />
                  ) : (
                    <div className="p-3 text-muted">
                      No hay registros usados.
