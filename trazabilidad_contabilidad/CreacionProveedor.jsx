...
const FEDERATION_OPTIONS = [
  {
    ...
    description:
      "FENALCE es la entidad administradora del Fondo Nacional Cerealista y del Fondo Nacional de Leguminosas y de Fríjol Soya. Se encarga de recaudar la Cuota de Fomento Cerealista.",
  },
  {
    id: "asohofrucol",
    label: "Asociación Hortifrutícola de Colombia (ASOHOFRUCOL)",
    description:
      "ASOHOFRUCOL es la entidad administradora del Fondo Nacional de Fomento Hortifrutícola. Se encarga de la Cuota de Fomento HortIfrutícola.",
  },
  {
    id: "fedepapa",
    label: "FEDERACIÓN COLOMBIANA DE PRODUCTORES DE PAPA – FEDEPAPA",
    description:
      "Entidad gremial de derecho privado, sin ánimo de lucro constituida por personas naturales y jurídicas que se dedican a la producción de papa en el país.",
  },
];
...
const CreacionProveedor = () => {
  ...
  const [certificadoBolsa, setCertificadoBolsa] = useState(null);
  const [certificadoFenalce, setCertificadoFenalce] = useState(null);
  const [certificadoAsohofrucol, setCertificadoAsohofrucol] = useState(null);
  const [certificadoFedepapa, setCertificadoFedepapa] = useState(null);
  ...
  useEffect(() => {
    // Cargar documentos públicos para proveedores
    const cargarDocumentos = async () => {
      try {
        const { data } = await apiTrazabilidad.get(
          `/trazabilidad/registro-publico/documentos/proveedor`,
        );
  ...
  const resetForm = () => {
    ...
    setCertificadoBolsa(null);
    setCertificadoFenalce(null);
    setCertificadoAsohofrucol(null);
    setCertificadoFedepapa(null);
    ...
    const inputs = [
      ...
      "certificado_bolsa",
      "certificado_fenalce",
      "certificado_asohofrucol",
      "certificado_fedepapa",
    ];
    inputs.forEach((id) => {
  ...
  const handleFederationChange = (event) => {
    ...
    setFormData((prev) => {
      const current = prev.federaciones_recaudo || [];
      if (checked) {
        return { ...prev, federaciones_recaudo: [...current, value] };
      } else {
        return {
          ...prev,
          federaciones_recaudo: current.filter((item) => item !== value),
        };
      }
    });
  };
  ...
  const buildProviderPayload = useCallback(() => {
    ...
    // Nota: Campos de contacto específicos abajo
    ...
    return {
      ...
      // Campos específicicos
      contacto_cartera_nombre: toNull(formData.contacto_cartera_nombre),
      contacto_cartera_email: toNull(formData.contacto_cartera_email),
      contacto_cartera_telefono: toNull(formData.contacto_cartera_telefono),
      contacto_tesoreria_nombre: toNull(formData.contacto_tesoreria_nombre),
      contacto_tesoreria_email: toNull(formData.contacto_tesoreria_email),
      contacto_tesoreria_telefono: toNull(formData.contacto_tesoreria_telefono),
      contacto_compras_nombre: toNull(formData.contacto_compras_nombre),
      contacto_compras_email: toNull(formData.contacto_compras_email),
      contacto_compras_telefono: toNull(formData.contacto_compras_telefono),
      ...
      // Nuevos campos
      registra_en_bolsa: toNull(formData.registra_en_bolsa),
      tipo_proveedor: toNull(formData.tipo_proveedor),
  ...
  const validateCompleteForm = useCallback(
    (validarDocumentos = false) => {
      ...
      if (validarDocumentos) {
        ...
        // Validar nuevos documentos condicionales
        if (formData.registra_en_bolsa === "si" && !certificadoBolsa) {
          errors.certificadoBolsa = "Debes adjuntar el certificado de bolsa.";
        }
        const federaciones = formData.federaciones_recaudo || [];
        if (federaciones.includes("fenalce") && !certificadoFenalce) {
          errors.certificadoFenalce = "Certificado FENALCE requerido.";
        }
        if (federaciones.includes("asohofrucol") && !certificadoAsohofrucol) {
          errors.certificadoAsohofrucol = "Certificado ASOHOFRUCOL requerido.";
        }
        if (federaciones.includes("fedepapa") && !certificadoFedepapa) {
          errors.certificadoFedepapa = "Certificado FEDEPAPA requerido.";
        }
      }
      ...
      if (!formData.tipo_proveedor)
        errors.tipo_proveedor = "Selecciona el tipo de proveedor.";
      ...
      // Validaciones de los nuevos campos de contacto
      // CARTERA
      if (!formData.contacto_cartera_nombre?.trim())
        errors.contacto_cartera_nombre = "Nombre contacto Cartera requerido";
      if (!formData.contacto_cartera_email?.trim()) {
        errors.contacto_cartera_email = "Email contacto Cartera requerido";
      } else if (!EMAIL_REGEX.test(formData.contacto_cartera_email)) {
        errors.contacto_cartera_email = "Email contacto Cartera inválido";
      }
      if (!formData.contacto_cartera_telefono?.trim())
        errors.contacto_cartera_telefono =
          "Teléfono contacto Cartera requerido";

      // TESORERIA
      if (!formData.contacto_tesoreria_nombre?.trim())
        errors.contacto_tesoreria_nombre =
          "Nombre contacto Tesorería requerido";
      if (!formData.contacto_tesoreria_email?.trim()) {
        errors.contacto_tesoreria_email = "Email contacto Tesorería requerido";
      } else if (!EMAIL_REGEX.test(formData.contacto_tesoreria_email)) {
        errors.contacto_tesoreria_email = "Email contacto Tesorería inválido";
      }
      if (!formData.contacto_tesoreria_telefono?.trim())
        errors.contacto_tesoreria_telefono =
          "Teléfono contacto Tesorería requerido";

      // COMPRAS
      if (!formData.contacto_compras_nombre?.trim())
        errors.contacto_compras_nombre = "Nombre contacto Compras requerido";
      if (!formData.contacto_compras_email?.trim()) {
        errors.contacto_compras_email = "Email contacto Compras requerido";
      } else if (!EMAIL_REGEX.test(formData.contacto_compras_email)) {
        errors.contacto_compras_email = "Email contacto Compras inválido";
      }
      if (!formData.contacto_compras_telefono?.trim())
        errors.contacto_compras_telefono =
          "Teléfono contacto Compras requerido";
    ...
    // PASO 2: Crear proveedor completo con datos y documentos
      ...
      const confirmResult = await Swal.fire({
        ...
        html: `
          <div style="text-align: left; margin-top: 1rem;">
            ...
            <p>Se adjuntarán los documentos cargados.</p>
          </div>
        `,
        icon: "question",
        customClass: SWAL_CUSTOM_CLASSES,
      ...
      try {
        ...
        const [
          ...
          uploadFileOrKeepUrl(certificadoBolsa, "certificado_bolsa"),
          uploadFileOrKeepUrl(certificadoFenalce, "certificado_fenalce"),
          uploadFileOrKeepUrl(
            certificadoAsohofrucol,
            "certificado_asohofrucol",
          ),
          uploadFileOrKeepUrl(certificadoFedepapa, "certificado_fedepapa"),
        ]);
        ...
        if (modoPublico) {
          await apiTrazabilidad.post(
            `/trazabilidad/registro-publico/proveedor/${tokenPublico}`,
            payloadForRequest,
          );
        } else {
          await apiTrazabilidad.post(
    ...
    <form onSubmit={handleFormSubmit} className="tc-form">
      ...
      {pasoActual === 1 && (
        <>
          ...
          <div className="tc-form-grid grid-2-cols">
            <div className="tc-form-group">
              <p className="tc-radio-label">
                ¿Registra en bolsa?<span className="required">*</span>
              </p>
              <div
                className={`tc-radio-group ${
                  formErrors.registra_en_bolsa ? "has-error" : ""
                }`}
              >
                {RESPUESTA_PREGUNTA.map((option) => (
                  <label key={option.value} className="tc-radio-option">
                    <input
                      type="radio"
                      name="registra_en_bolsa"
                      value={option.value}
                      checked={formData.registra_en_bolsa === option.value}
                      onChange={handleFieldChange}
                    />
                    {option.label}
                  </label>
            ...
            <div className="tc-form-group">
              ...
              <select
                ...
                onChange={handleFieldChange}
                className={`tc-form-input ${
                  formErrors.tipo_proveedor ? "is-invalid" : ""
                }`}
              >
                <option value="">Seleccione...</option>
                {PROVEEDOR_TYPE_OPTIONS.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
                ))}
              </select>
              {formErrors.tipo_proveedor && (
                <span className="tc-validation-error">
                  {formErrors.tipo_proveedor}
                </span>
              )}
          ...
          <div className="tc-form-group" style={{ marginTop: "1.5rem" }}>
            <p className="tc-radio-label">
              ¿Recauda con alguna de estas federaciones? (Seleccione las que
              apliquen)
            </p>
            <div className="tc-federation-group">
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      handleFederationChange(event);
                    }}
                  >
                    <div className="tc-federation-checkbox">
                      <FaCheck />
                    </div>
                    <div className="tc-federation-content">
                      <div className="tc-federation-title">{option.label}</div>
                      <p className="tc-federation-desc">{option.description}</p>
                    </div>
                  </div>
                );
              })}
          ...
          {/* CONTACTO CARTERA */}
          <div className="tc-form-separator">
            <span>Contacto y Notificación - Cartera</span>
          </div>
          <div className="tc-form-grid grid-3-cols">
            <div className="tc-form-group">
              <label htmlFor="contacto_cartera_nombre">
                Nombre<span className="required">*</span>
              </label>
              <input
                id="contacto_cartera_nombre"
                name="contacto_cartera_nombre"
                type="text"
                value={formData.contacto_cartera_nombre}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_cartera_nombre ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_cartera_nombre && (
                <span className="tc-validation-error">
                  {formErrors.contacto_cartera_nombre}
                </span>
              )}
            ...
            <div className="tc-form-group">
              <label htmlFor="contacto_cartera_email">
                Email<span className="required">*</span>
              </label>
              <input
                id="contacto_cartera_email"
                name="contacto_cartera_email"
                type="email"
                value={formData.contacto_cartera_email}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_cartera_email ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_cartera_email && (
                <span className="tc-validation-error">
                  {formErrors.contacto_cartera_email}
                </span>
              )}
            ...
            <div className="tc-form-group">
              <label htmlFor="contacto_cartera_telefono">
                Teléfono<span className="required">*</span>
              </label>
              <input
                id="contacto_cartera_telefono"
                name="contacto_cartera_telefono"
                type="tel"
                value={formData.contacto_cartera_telefono}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_cartera_telefono ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_cartera_telefono && (
                <span className="tc-validation-error">
                  {formErrors.contacto_cartera_telefono}
                </span>
              )}
          ...
          {/* CONTACTO TESORERIA */}
          <div className="tc-form-separator">
            <span>Contacto y Notificación - Tesorería</span>
          </div>
          <div className="tc-form-grid grid-3-cols">
            <div className="tc-form-group">
              <label htmlFor="contacto_tesoreria_nombre">
                Nombre<span className="required">*</span>
              </label>
              <input
                id="contacto_tesoreria_nombre"
                name="contacto_tesoreria_nombre"
                type="text"
                value={formData.contacto_tesoreria_nombre}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_tesoreria_nombre ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_tesoreria_nombre && (
                <span className="tc-validation-error">
                  {formErrors.contacto_tesoreria_nombre}
                </span>
              )}
            ...
            <div className="tc-form-group">
              <label htmlFor="contacto_tesoreria_email">
                Email<span className="required">*</span>
              </label>
              <input
                id="contacto_tesoreria_email"
                name="contacto_tesoreria_email"
                type="email"
                value={formData.contacto_tesoreria_email}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_tesoreria_email ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_tesoreria_email && (
                <span className="tc-validation-error">
                  {formErrors.contacto_tesoreria_email}
                </span>
              )}
            ...
            <div className="tc-form-group">
              <label htmlFor="contacto_tesoreria_telefono">
                Teléfono<span className="required">*</span>
              </label>
              <input
                id="contacto_tesoreria_telefono"
                name="contacto_tesoreria_telefono"
                type="tel"
                value={formData.contacto_tesoreria_telefono}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_tesoreria_telefono ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_tesoreria_telefono && (
                <span className="tc-validation-error">
                  {formErrors.contacto_tesoreria_telefono}
                </span>
              )}
          ...
          {/* CONTACTO COMPRAS */}
          <div className="tc-form-separator">
            <span>Contacto y Notificación - Compras</span>
          </div>
          <div className="tc-form-grid grid-3-cols">
            <div className="tc-form-group">
              <label htmlFor="contacto_compras_nombre">
                Nombre<span className="required">*</span>
              </label>
              <input
                id="contacto_compras_nombre"
                name="contacto_compras_nombre"
                type="text"
                value={formData.contacto_compras_nombre}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_compras_nombre ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_compras_nombre && (
                <span className="tc-validation-error">
                  {formErrors.contacto_compras_nombre}
                </span>
              )}
            ...
            <div className="tc-form-group">
              <label htmlFor="contacto_compras_email">
                Email<span className="required">*</span>
              </label>
              <input
                id="contacto_compras_email"
                name="contacto_compras_email"
                type="email"
                value={formData.contacto_compras_email}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_compras_email ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_compras_email && (
                <span className="tc-validation-error">
                  {formErrors.contacto_compras_email}
                </span>
              )}
            ...
            <div className="tc-form-group">
              <label htmlFor="contacto_compras_telefono">
                Teléfono<span className="required">*</span>
              </label>
              <input
                id="contacto_compras_telefono"
                name="contacto_compras_telefono"
                type="tel"
                value={formData.contacto_compras_telefono}
                onChange={handleFieldChange}
                className={`tc-form-input ${formErrors.contacto_compras_telefono ? "is-invalid" : ""}`}
              />
              {formErrors.contacto_compras_telefono && (
                <span className="tc-validation-error">
                  {formErrors.contacto_compras_telefono}
                </span>
              )}
      ...
      {pasoActual === 2 && (
        <>
          ...
          <div className="tc-form-grid">
            ...
            {/* Documentos Condicionales Prerequisitos */}
            {formData.registra_en_bolsa === "si" && (
              <FileInput
                label="Certificado de Bolsa"
                name="certificado_bolsa"
                file={certificadoBolsa}
                setFile={setCertificadoBolsa}
                isRequired={true}
              />
            )}

            {(formData.federaciones_recaudo || []).includes("fenalce") && (
              <FileInput
                label="Certificado FENALCE"
                name="certificado_fenalce"
                file={certificadoFenalce}
                setFile={setCertificadoFenalce}
                isRequired={true}
              />
            )}

            {(formData.federaciones_recaudo || []).includes("asohofrucol") && (
              <FileInput
                label="Certificado ASOHOFRUCOL"
                name="certificado_asohofrucol"
                file={certificadoAsohofrucol}
                setFile={setCertificadoAsohofrucol}
                isRequired={true}
              />
            )}

            {(formData.federaciones_recaudo || []).includes("fedepapa") && (
              <FileInput
                label="Certificado FEDEPAPA"
                name="certificado_fedepapa"
                file={certificadoFedepapa}
                setFile={setCertificadoFedepapa}
                isRequired={true}
              />
            )}
          ...
          {(formErrors.rut ||
            ...
            formErrors.certificadoBolsa ||
            formErrors.certificadoFenalce ||
            formErrors.certificadoAsohofrucol ||
            formErrors.certificadoFedepapa) && (
            <div className="tc-error-summary">
              ...
              {formErrors.certificadoBolsa && (
                <span className="tc-validation-error">
                  ⚠️ {formErrors.certificadoBolsa}
                </span>
              )}
              {formErrors.certificadoFenalce && (
                <span className="tc-validation-error">
                  ⚠️ {formErrors.certificadoFenalce}
                </span>
              )}
              {formErrors.certificadoAsohofrucol && (
                <span className="tc-validation-error">
                  ⚠️ {formErrors.certificadoAsohofrucol}
                </span>
              )}
              {formErrors.certificadoFedepapa && (
                <span className="tc-validation-error">
                  ⚠️ {formErrors.certificadoFedepapa}
                </span>
              )}
            </div>
          )}
          ...
          <div className="tc-form-actions">
            ...
            <button
              ...
              disabled={
                ...
                !certificadoSagrilaft ||
                !composicionAccionaria ||
                (formData.registra_en_bolsa === "si" && !certificadoBolsa) ||
                ((formData.federaciones_recaudo || []).includes("fenalce") &&
                  !certificadoFenalce) ||
                ((formData.federaciones_recaudo || []).includes(
                  "asohofrucol",
                ) &&
                  !certificadoAsohofrucol) ||
                ((formData.federaciones_recaudo || []).includes("fedepapa") &&
