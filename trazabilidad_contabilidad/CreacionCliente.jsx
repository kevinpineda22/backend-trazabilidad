...
const CreacionCliente = () => {
  ...
  useEffect(() => {
    ...
    const cargarDocumentos = async () => {
      try {
        const { data } = await apiTrazabilidad.get(
          `/trazabilidad/registro-publico/documentos/cliente`,
        );
  ...
  useEffect(() => {
    setFormErrors((prev) => {
      ...
      if (cedula && prev.cedula) {
        nextErrors.cedula = undefined;
        hasChanges = true;
      }
  ...
  const validateCompleteForm = useCallback(
    (validarDocumentos = false) => {
      ...
      if (validarDocumentos) {
        ...
        if (!cedula) errors.cedula = "Cédula es requerida.";
        if (!certificadoSagrilaft)
          errors.certificadoSagrilaft =
            "Certificado SAGRILAFT firmado por el oficial de cumplimiento es requerido.";
      ...
      // Nombre de contacto obligatorio para ambos (Jurídica y Natural)
  ...
  const handleFormSubmit = async (e) => {
    ...
    // PASO 2: Crear proveedor completo con datos y documentos
      ...
      const folderPath = `${FOLDER_BASE}/cliente_${Date.now()}_${Math.random()
        .toString(36)
        .substring(2, 8)}`;
      ...
      try {
        ...
        const [
          ...
          uploadFileOrKeepUrl(cedula, "cedula"),
        ...
        if (modoPublico) {
          await apiTrazabilidad.post(
            `/trazabilidad/registro-publico/cliente/${tokenPublico}`,
            payloadForRequest,
          );
        } else {
          await apiTrazabilidad.post(
      ...
      {pasoActual === 2 && (
        <>
          ...
          <div className="tc-form-grid">
            ...
            />
            <FileInput
              label="Certificado SAGRILAFT firmado por el oficial de cumplimiento"
  ...
  // --- (Renderizado final actualizado para clientes) ---
  return (
    <>
      <FilePreviewModal
        isOpen={modalOpen}
      ...
      <TrazabilidadPageLayout
        ...
        icon={
          pasoActual === 2 ? (
            ...
            <FaUserTie style={{ fontSize: "1.25em" }} />
          )
        }
        subtitle={
          pasoActual === 1
