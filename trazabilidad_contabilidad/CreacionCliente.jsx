...
const CreacionCliente = () => {
  ...
  const validateForm = () => {
    ...
    if (!camaraComercio)
      errors.camaraComercio = "La Cámara de Comercio es requerida.";
    if (!formatoSangrilaft)
      errors.formatoSangrilaft = "El Formato Sangrilaft es requerido."; // Renombrado
    ...
    try {
      ...
      if (modoPublico) {
        await axios.post(
          `${
            import.meta.env.VITE_BACKEND_TRAZABILIDAD_URL
          }/api/trazabilidad/registro-publico/cliente/${tokenPublico}`,
          finalPayload,
      ...
      // Modo normal: enviar directo a clientes
        ...
        if (enModoEdicion) {
          await apiTrazabilidad.patch(
            `/trazabilidad/clientes/${idParaEditar}`,
            finalPayload,
            {
              headers: { "Content-Type": "application/json" },
            }
      ...
      console.error(
        `Error al ${enModoEdicion ? "actualizar" : "crear"} cliente:`,
        error
      );
    ...
    },
    {
      ...
      cell: (item) => (
        <div className="tc-doc-list">
          ...
          />
          <DocLink
            url={item.url_formato_sangrilaft}
            label="F. Sangrilaft"
            onPreview={openPreview}
          />
          <DocLink
    ...
    <form onSubmit={handleFormSubmit} className="tc-form">
      ...
      {(formErrors.rut ||
        ...
        <div className="tc-error-summary">
          ...
          {formErrors.formatoSangrilaft && (
            <span className="tc-validation-error">
              ⚠️ {formErrors.formatoSangrilaft}
            </span>
          )}
          {formErrors.cedulaFile && (
            <span className="tc-validation-error">
  ...
  return (
    <>
      ...
      <TrazabilidadPageLayout
        ...
        subtitle={
          enModoEdicion
            ? `Editando cliente (Cupo: ${cupo}, Plazo: ${plazo})`
            : "Adjunta los datos y documentos para registrar un nuevo cliente."
        }
