...
import {
    FaUserPlus, FaUser, FaUpload, FaFileAlt, FaTimesCircle, FaFilePdf, FaFileImage, FaHistory, FaSpinner, FaInfoCircle, FaSort, FaSortUp, FaSortDown,
    FaEdit, 
} from "react-icons/fa";
...
const validateEmail = (email) => {
    ...
    if (!email) return "";
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return re.test(String(email).toLowerCase()) ? "" : "Correo electrónico no válido.";
};
...
const CreacionSubirEmpleado = () => {
    ...
    const fetchHistorial = useCallback(async () => {
        ...
        try {
            const { data } = await apiTrazabilidad.get("/trazabilidad/empleados/historial");
            setHistorial(data || []);
        } catch (error) {
    ...
    const resetForm = () => {
        ...
        const inputs = ["hoja_de_vida", "cedula_file", "certificado_bancario", "habeas_data"]; 
        inputs.forEach((id) => {
    ...
    const handleApiSubmit = async (payload, confirmationDetails, successMessage) => {
        ...
        try {
            const safeFolderName = `CC${payload.cedula}_${payload.nombre}_${payload.apellidos}`
                .replace(/[^a-zA-Z0-9 ]/g, '')
                .replace(/ /g, '_')
                .toUpperCase();
    ...
    useEffect(() => { setFormErrors(errors => ({ ...errors, correo: correo ? validateEmail(correo) : "" })); }, [correo]);
    useEffect(() => { setFormErrors(errors => ({ ...errors, contacto: contacto ? validatePhone(contacto) : "" })); }, [contacto]);
    ...
    const validateForm = () => {
        ...
        if (!certificadoBancario) errors.certificadoBancario = "Certificado bancario es requerido.";
    ...
    const handleFormSubmit = async (e) => {
        ...
        const payload = {
            nombre, apellidos, cedula: cedulaInput, contacto,
            correo_electronico: correo, direccion, codigo_ciudad: codigoCiudad,
        };
    ...
    const columns = [
        { 
            ...
            cell: (item) => (<div className="tc-user-cell"><FaUser />{item.profiles?.nombre || "N/A"}</div>), 
        },
        { 
        ...
        },
        {
            ...
            cell: (item) => (
                <div className="tc-doc-list">
                    <DocLink url={item.url_hoja_de_vida} label="Hoja de Vida" onPreview={openPreview} />
                    <DocLink url={item.url_cedula} label="Cédula" onPreview={openPreview} />
                    <DocLink url={item.url_certificado_bancario} label="Cert. Bancario" onPreview={openPreview} />
                    <DocLink url={item.url_habeas_data} label="Habeas Data" onPreview={openPreview} /> 
                </div>
            ),
        },
        {
        ...
        <form onSubmit={handleFormSubmit} className="tc-form">
            ...
            <div className="tc-form-grid">
                ...
                <div className="tc-form-group">
                    ...
                    <input type="text" id="nombre" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Ana" required className={`tc-form-input ${formErrors.nombre ? 'is-invalid' : ''}`} />
                    {formErrors.nombre && <span className="tc-validation-error">{formErrors.nombre}</span>}
                </div>
                <div className="tc-form-group">
                    <label htmlFor="apellidos">Apellidos <span className="required">*</span></label>
                    <input type="text" id="apellidos" value={apellidos} onChange={(e) => setApellidos(e.target.value)} placeholder="Ej: Pérez" required className={`tc-form-input ${formErrors.apellidos ? 'is-invalid' : ''}`} />
                    {formErrors.apellidos && <span className="tc-validation-error">{formErrors.apellidos}</span>}
                </div>
                <div className="tc-form-group">
                    <label htmlFor="cedula_input">Cédula (N°) <span className="required">*</span></label>
                    <input type="text" id="cedula_input" value={cedulaInput} onChange={(e) => setCedulaInput(e.target.value)} placeholder="Ej: 10203040" required className={`tc-form-input ${formErrors.cedulaInput ? 'is-invalid' : ''}`} />
                    {formErrors.cedulaInput && <span className="tc-validation-error">{formErrors.cedulaInput}</span>}
                </div>
                
                {/* Fila 2 */}
                <div className="tc-form-group">
                    <label htmlFor="contacto">Contacto (Celular)</label>
                    <input type="tel" id="contacto" value={contacto} onChange={(e) => setContacto(e.target.value)} placeholder="Ej: 3001234567" className={`tc-form-input ${formErrors.contacto ? 'is-invalid' : ''}`} />
                    {formErrors.contacto && <span className="tc-validation-error">{formErrors.contacto}</span>}
                </div>
                <div className="tc-form-group">
                    <label htmlFor="correo">Correo Electrónico</label>
                    <input type="email" id="correo" value={correo} onChange={(e) => setCorreo(e.target.value)} placeholder="Ej: ana.perez@email.com" className={`tc-form-input ${formErrors.correo ? 'is-invalid' : ''}`} />
                    {formErrors.correo && <span className="tc-validation-error">{formErrors.correo}</span>}
                </div>
                <div className="tc-form-group">
                    <label htmlFor="direccion">Dirección</label>
                    <input type="text" id="direccion" value={direccion} onChange={(e) => setDireccion(e.target.value)} placeholder="Ej: Calle 50 # 45 - 30" className="tc-form-input" />
                </div>
                ...
                <div className="tc-form-group full-width">
                    <div className="tc-form-grid grid-2-cols">
                        <div className="tc-form-group">
                            ...
                            <input type="text" id="codigo_pais" value="169" readOnly disabled className="tc-form-input tc-readonly-input" />
                        </div>
                        <div className="tc-form-group">
                            <label htmlFor="codigo_dpto">Código Departamento</label>
                            <input type="text" id="codigo_dpto" value="05" readOnly disabled className="tc-form-input tc-readonly-input" />
                        </div>
                        <div className="tc-form-group">
                            <label htmlFor="codigo_ciudad">Código Ciudad</label>
                            <input type="text" id="codigo_ciudad" value={codigoCiudad} onChange={(e) => setCodigoCiudad(e.target.value)} placeholder="Ej: 001 (Medellín)" className="tc-form-input" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="tc-form-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
                ...
                <FileInput label="Habeas Data" name="habeas_data" file={habeasData} setFile={setHabeasData} isRequired={true} />
            </div>
            
            {/* Errores de validación */}
            { (formErrors.hojaDeVida || formErrors.cedulaFile || formErrors.certificadoBancario || formErrors.habeasData) &&
                <div className="tc-error-summary">
                    {formErrors.hojaDeVida && <span className="tc-validation-error">⚠️ {formErrors.hojaDeVida}</span>}
                    {formErrors.cedulaFile && <span className="tc-validation-error">⚠️ {formErrors.cedulaFile}</span>}
                    {formErrors.certificadoBancario && <span className="tc-validation-error">⚠️ {formErrors.certificadoBancario}</span>}
                    {formErrors.habeasData && <span className="tc-validation-error">⚠️ {formErrors.habeasData}</span>}
                </div>
            }
            ...
            <div className="tc-form-actions">
                ...
                <button 
                    ...
                    disabled={loading || isSubmitting || !hojaDeVida || !cedulaFile || !certificadoBancario || !habeasData} 
                >
                    {loading || isSubmitting ? <FaSpinner className="tc-spinner" /> : (enModoEdicion ? <FaEdit /> : <FaUserPlus />)}
                    {loading || isSubmitting ? (enModoEdicion ? "Actualizando..." : "Guardando...") : (enModoEdicion ? "Actualizar Empleado" : "Guardar Empleado")}
                </button>
            </div>
        </form>
    );
    ...
    return (
        <>
            ...
            <TrazabilidadPageLayout
                title={enModoEdicion ? "Editar Empleado (Contabilidad)" : "Crear Nuevo Empleado (Contabilidad)"}
                icon={enModoEdicion ? <FaEdit style={{fontSize: '1.25em'}} /> : <FaUserPlus style={{fontSize: '1.25em'}} />}
                subtitle={enModoEdicion ? `Editando a: ${nombre} ${apellidos} (CC: ${cedulaInput})` : "Diligencia los datos y adjunta los documentos requeridos para el ingreso del nuevo empleado."}
