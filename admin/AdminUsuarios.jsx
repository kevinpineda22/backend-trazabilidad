...
export const AdminUsuarios = () => {
  ...
  const cargarUsuarios = async () => {
    ...
    const { data, error } = await supabaseQuery(
      () =>
        supabase
          .from("profiles")
          .select(
            "user_id, nombre, area, correo, company, role, personal_routes, proceso"
          ) // ✅ AGREGAR proceso
    );
    ...
    if (error) {
      ...
      console.log("📋 Usuarios cargados:", data); // ✅ Verificar que proceso viene en los datos
      setUsuarios(data || []);
    }
    setLoading(false);
  };
  ...
  // Exportar a CSV
  const exportToCSV = () => {
    const headers = ["Nombre", "Correo", "Área", "Empresa", "Rol"];
